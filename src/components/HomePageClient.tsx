...
import { ArrowRight, Shield, Zap, CreditCard, Phone, Mail, MapPin, Star, ChevronRight, Loader2 } from 'lucide-react';

interface Service { id: number; name: string; description: string | null; price: string | null; priceType: string | null; }
interface Testimonial { id: number; name: string; role: string | null; content: string; rating: number | null; }
...
export default function HomePageClient() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      ...
      .catch(err => {
        console.error('HomePageClient: Failed to load settings:', err);
        setConfig({
          ...
          logo: '', heroImage: '', primaryColor: '#FF3366', accentColor: '#00D4FF',
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl md:text-7xl font-black leading-[1.05] tracking-tight mb-6 animate-fade-in animate-delay-1">
              {(config.tagline || 'Professional services you can trust').split(' ').map((word: string, i: number, arr: string[]) => (
                i >= arr.length - 2 ? (
